/*
 * Proyecto : LAB1-2023_2-P2-B
 * Archivo  : main.cpp
 * Autor    : Alex Calero Revilla
 * Codigo   : 20206455
 * Fecha    : 31 de marzo de 2024, 21:10
 */
...
//	}
	...
	int beneficioMaximo = 0, beneficioParcial;
	int cromo[n];
	int comb = pow(2, n);
	int numMejorSol, mejorSol, numSol = 0;
	for(int i = 0; i < comb; i++) {
		...
		for(int j = 0; j < n; j++) {
			if(cromo[j] == 1) {
				...
				beneficioParcial += arrAntivirus[j].beneficio;
			}
		}
		if(inversionParcial == p && cumplePredecesores(arrAntivirus, cromo, n)) {
			...
			for(int j = 0; j < n; j++)
				if(cromo[j])
					cout << j + 1 << " ";
			cout << "\nBeneficio generado: " << beneficioParcial << ",000\n" << endl;
			if(beneficioParcial > beneficioMaximo) {
				beneficioMaximo = beneficioParcial;
				numMejorSol = numSol;
				mejorSol = i;
			}
		}
	...
	cout << "La solucion " << numMejorSol << " es la que genera el beneficio maximo:\n";
	cargaBin(mejorSol, cromo, n);
	cout << "Antivirus seleccionados: ";
	for(int i = 0; i < n; i++)
		if(cromo[i] == 1)
			cout << i + 1 << " ";
	cout << "\nBeneficio maximo: " << beneficioMaximo << ",000\n";
