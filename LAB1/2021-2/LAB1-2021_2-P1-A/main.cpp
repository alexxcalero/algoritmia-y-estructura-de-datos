/*
 * Proyecto : LAB1-2021_2-P1-A
 * Archivo  : main.cpp
 * Autor    : Alex Calero Revilla
 * Codigo   : 20206455
 * Fecha    : 1 de abril de 2024, 00:40
 */
		...
		for(int j = 0; j < comb; j++) {
			...
			for(int k = 0; k < comb; k++) {
				...
				for(int l = 0; l < comb; l++) {
					...
					pesoParcial = 0;
					for(int m = 0; m < n; m++) {
						if(cromo1[m] == 1) pesoParcial += arrProductos[0][m].peso;
						if(cromo2[m] == 1) pesoParcial += arrProductos[1][m].peso;
						if(cromo3[m] == 1) pesoParcial += arrProductos[2][m].peso;
						if(cromo4[m] == 1) pesoParcial += arrProductos[3][m].peso;
					}
					if(pesoParcial == p) {
						numSol++;
						cout << "Resultado " << numSol << ": ";
						for(int m = 0; m < n; m++) {
