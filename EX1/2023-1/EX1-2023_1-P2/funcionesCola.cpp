...
void encolar(struct Cola & cola, int prioridad, char *codigo){
	Elemento elemento;
	elemento.prioridad = prioridad;
	strcpy(elemento.codigo, codigo);
    insertarencola(cola.lista, elemento);
	//encolarPrioridad(cola.lista, elemento);
}

void encolarPrioridad(struct Cola & cola, int prioridad, char *codigo){
	Elemento elemento;
	elemento.prioridad = prioridad;
	strcpy(elemento.codigo, codigo);
    //insertarencola(cola.lista, elemento);
...
Elemento desencolar(struct Cola & cola){
    ...
    if(esColaVacia(cola)){
		...
		strcpy(elemento.codigo, "\0");
        return elemento; //Cero o algún elemento que no esté en la cola
    } else {
        elemento = retornaCabeza(cola.lista);
